...
public class SavingsAccountDomainServiceJpa implements SavingsAccountDomainService {
    ...
    public SavingsAccountTransaction handleWithdrawal(final SavingsAccount account, final DateTimeFormatter fmt,
        ...
        final boolean canUseFastPath = !skipBalanceValidation
                && account.canSkipInterestRecalculation(transactionDate, backdatedTxnsAllowedTill);
        final boolean canUseIncrementalPath = !skipBalanceValidation
                && account.canUseIncrementalRecalculation(transactionDate, backdatedTxnsAllowedTill);
        ...
        if (canUseFastPath) {
            ...
            if (!skipBalanceValidation) {
                account.validateAccountBalanceDoesNotBecomeNegative(transactionAmount,
                        transactionBooleanValues.isExceptionForBalanceCheck(), depositAccountOnHoldTransactions, backdatedTxnsAllowedTill);
            }
